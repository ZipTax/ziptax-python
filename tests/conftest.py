...
def _build_v60_response(
    normalized_address: str,
    sourcing_value: str = "D",
    rate: float = 0.0775,
) -> dict:
    """Build a V60 response dict with configurable address, sourcing, and rate.

    This is a helper function (not a fixture) so tests can call it with
    different parameters inline.
    """
    return {
        "metadata": {
            "version": "v60",
        ...
        "baseRates": [
            {
                ...
                "jurType": "US_STATE_SALES_TAX",
                "jurName": (
                    normalized_address.split(",")[-3].strip().split()[0]
        ...
        "sourcingRules": {
            ...
            "description": (
                ...
                if sourcing_value == "O"
                else "Destination Based Taxation"
            ),
            "value": sourcing_value,
        },
        "taxSummaries": [
            {
                "rate": rate,
                "taxType": "SALES_TAX",
                "summaryName": "Total Base Sales Tax",
                "displayRates": [{"name": "Total Rate", "rate": rate}],
            }
        ],
        "addressDetail": {
...
@pytest.fixture
def v60_ca_destination():
    """V60 response for a California destination-based address."""
    return _build_v60_response(
        normalized_address=(
            "200 Spectrum Center Dr, Irvine, CA 92618-5003, United States"
        ),
        sourcing_value="D",
...
@pytest.fixture
def v60_mn_destination():
    """V60 response for a Minnesota destination-based address."""
    return _build_v60_response(
        normalized_address=(
            "323 Washington Ave N, Minneapolis, MN 55401-2427, United States"
        ),
        sourcing_value="D",
        rate=0.08025,
    )
...
@pytest.fixture
def v60_tx_origin_dallas():
    """V60 response for a Texas origin-based address (Dallas)."""
    return _build_v60_response(
        normalized_address=("123 Main St, Dallas, TX 75201-1234, United States"),
        sourcing_value="O",
        rate=0.08250,
    )
...
@pytest.fixture
def v60_tx_origin_austin():
    """V60 response for a Texas origin-based address (Austin)."""
    return _build_v60_response(
        normalized_address=("456 Elm St, Austin, TX 78701-5678, United States"),
        sourcing_value="O",
        rate=0.08250,
    )
